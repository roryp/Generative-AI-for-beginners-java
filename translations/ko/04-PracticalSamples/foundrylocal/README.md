...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-21T16:34:58+00:00",
...
>**Note**: 이 장에는 완성된 샘플 실행 과정을 안내하는 [**튜토리얼**](./TUTORIAL.md)이 포함되어 있습니다.
...
- [사전 요구사항](../../../../04-PracticalSamples/foundrylocal)
...
- [코드 주요 내용](../../../../04-PracticalSamples/foundrylocal)
...
> **⚠️ Note**: 이 애플리케이션은 **제공된 devcontainer에서 실행되지 않습니다**. Foundry Local이 호스트 시스템에 설치되고 실행 중이어야 합니다.

### Foundry Local 설치

이 애플리케이션을 실행하기 전에 Foundry Local을 설치하고 시작해야 합니다. 다음 단계를 따르세요:
...
3. **첫 번째 모델 시작**:
   ...
   모델이 다운로드된 후 (인터넷 속도에 따라 몇 분이 소요될 수 있음) 실행됩니다. Foundry Local은 시스템에 가장 적합한 모델 변형(CUDA는 NVIDIA GPU용, CPU 버전은 그 외)을 자동으로 선택합니다.
...
4. **모델 테스트**: 동일한 터미널에서 질문을 입력하세요:
   ...
   하늘이 왜 파랗게 보이는지 설명하는 Phi 모델의 응답을 확인할 수 있습니다.
...
> **Note**: 구성에 입력된 모델 이름은 Foundry Local이 시스템에 다운로드한 특정 변형과 일치해야 합니다. `foundry model run phi-3.5-mini`를 실행하면 Foundry Local이 자동으로 최적의 변형(CUDA는 NVIDIA GPU용, CPU 버전은 그 외)을 선택하고 다운로드합니다. `foundry model list`를 사용하여 로컬 인스턴스에서 사용할 수 있는 정확한 모델 이름을 확인하세요.
...
1. **명령줄 애플리케이션**으로 시작 (웹 서버 없음)
2. **테스트 메시지 전송**: "Hello! Can you tell me what you are and what model you're running?"
3. **Foundry Local의 응답**을 콘솔에 표시
4. **데모 종료 후** 깨끗하게 종료
...
- SDK를 통한 자동 JSON 직렬화/역직렬화
- Spring의 `@Value` 및 `@PostConstruct` 어노테이션을 사용한 간단한 구성

## 코드 주요 내용

### OpenAI Java SDK 통합

애플리케이션은 OpenAI Java SDK를 사용하여 Foundry Local에 맞게 구성된 클라이언트를 생성합니다:
...
연결 오류가 발생하면:
1. Foundry Local이 `http://localhost:5273`에서 실행 중인지 확인
2. `foundry model list`를 사용하여 Phi-3.5-mini 모델 변형이 사용 가능한지 확인
3. `application.properties`의 모델 이름이 목록에 표시된 정확한 모델 이름과 일치하는지 확인
4. 방화벽이 연결을 차단하지 않는지 확인

일반적인 문제:
- **모델을 찾을 수 없음**: `foundry model run phi-3.5-mini`를 실행하여 모델을 다운로드하고 시작
- **서비스가 실행 중이지 않음**: Foundry Local 서비스가 중지되었을 수 있음; 모델 실행 명령으로 다시 시작
- **잘못된 모델 이름**: `foundry model list`를 사용하여 사용 가능한 모델을 확인하고 구성을 업데이트
