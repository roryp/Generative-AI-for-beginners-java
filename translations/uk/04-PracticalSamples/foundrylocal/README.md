...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-25T10:19:13+00:00",
...
>**Примітка**: Цей розділ включає [**Підручник**](./TUTORIAL.md), який допоможе вам ознайомитися з прикладами.
...
- Найкращі практики для локальної розробки та тестування штучного інтелекту
...
Перед запуском цього додатка необхідно встановити та запустити Foundry Local. Виконайте наступні кроки:
...
2. **Встановіть Foundry Local**:
   ...
   **Для macOS:**
   ...
   Альтернативно, ви можете завантажити інсталятор із [репозиторію Foundry Local на GitHub](https://github.com/microsoft/Foundry-Local).
...
3. **Запустіть першу модель**:
   ...
   Модель завантажиться (це може зайняти кілька хвилин залежно від швидкості вашого інтернету) і почне працювати. Foundry Local автоматично вибирає найкращий варіант моделі для вашої системи (CUDA для NVIDIA GPU, версія для CPU в іншому випадку).

4. **Перевірте модель**, поставивши запитання в тому ж терміналі:
...
Цей додаток використовує OpenAI Java SDK для взаємодії з Foundry Local. Основна залежність:
...
- **FoundryLocalService.java** - Сервіс, який використовує OpenAI Java SDK для взаємодії з Foundry Local
- Використовує **OpenAI Java SDK** для типобезпечних викликів API
- Автоматична серіалізація/десеріалізація JSON, що обробляється SDK
...
Запити на завершення чату виконуються просто та типобезпечно:
...
2. Перевірте, чи доступна модель Phi-3.5-mini за допомогою команди `foundry model list`
3. Переконайтеся, що назва моделі у файлі `application.properties` відповідає точній назві моделі, показаній у списку
...
- **Неправильна назва моделі**: Використовуйте `foundry model list`, щоб побачити доступні моделі, і оновіть конфігурацію відповідно
