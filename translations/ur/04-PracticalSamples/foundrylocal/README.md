...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-03T19:57:54+00:00",
...
**یہ کیا کرتا ہے:**
- **base-url**: یہ بتاتا ہے کہ فاؤنڈری لوکل کہاں چل رہا ہے، اور `/v1` راستہ OpenAI API کے ساتھ مطابقت کے لیے شامل کرتا ہے۔ **نوٹ:** فاؤنڈری لوکل پورٹ کو ڈائنامکلی اسائن کرتا ہے، لہذا اپنی اصل پورٹ `foundry service status` کے ذریعے چیک کریں۔
...
**اہم تصور:** اسپرنگ بوٹ خود بخود ان پراپرٹیز کو لوڈ کرتا ہے اور آپ کی ایپلیکیشن میں `@Value` اینوٹیشن کے ذریعے دستیاب بناتا ہے۔
...
- `@Value` application.properties سے کنفیگریشن ویلیوز کو انجیکٹ کرتا ہے
- `:default-value` سینٹیکس پراپرٹیز سیٹ نہ ہونے پر فال بیک ویلیوز فراہم کرتا ہے

#### کلائنٹ انیشیالائزیشن:
```java
@PostConstruct
public void init() {
...
- `application.properties` سے بیس URL پہلے ہی `/v1` شامل کرتا ہے OpenAI API کے ساتھ مطابقت کے لیے
...
- **ChatCompletionCreateParams**: اے آئی درخواست کو کنفیگر کرتا ہے
  ...
  - `maxTokens`: جواب کی لمبائی محدود کرتا ہے (وسائل بچانے کے لیے)
...
**یہ کیا کرتے ہیں:**
- **spring-boot-starter**: اسپرنگ بوٹ کی بنیادی خصوصیات فراہم کرتا ہے
...
3. **کلائنٹ سیٹ اپ**: `@PostConstruct` OpenAI کلائنٹ کو فاؤنڈری لوکل سے کنیکٹ کرنے کے لیے انیشیالائز کرتا ہے
...
6. **API درخواست**: سروس OpenAI-مطابقت پذیر درخواست فاؤنڈری لوکل کو بھیجتی ہے
...
2. **ڈائنامکلی اسائن کردہ پورٹ چیک کریں**۔ فاؤنڈری لوکل خود بخود پورٹ اسائن کرتا ہے جب یہ شروع ہوتا ہے۔ اپنی پورٹ معلوم کریں:
   ```bash
   foundry service status
   ```
   
   **اختیاری**: اگر آپ مخصوص پورٹ (جیسے 5273) استعمال کرنا چاہتے ہیں، تو آپ اسے دستی طور پر کنفیگر کر سکتے ہیں:
   ```bash
   foundry service set --port 5273
   ```


3. **اے آئی ماڈل ڈاؤنلوڈ کریں** جو آپ استعمال کرنا چاہتے ہیں، مثال کے طور پر، `phi-3.5-mini`، درج ذیل کمانڈ کے ساتھ:
   ```bash
   foundry model run phi-3.5-mini
   ```
...
- چیک کریں کہ فاؤنڈری لوکل کون سی اصل پورٹ استعمال کر رہا ہے: `foundry service status`
- اپنی `application.properties` کو درست پورٹ کے ساتھ اپڈیٹ کریں، اس بات کو یقینی بنائیں کہ URL آخر میں `/v1` پر ختم ہو
- متبادل طور پر، اگر چاہیں تو مخصوص پورٹ سیٹ کریں: `foundry service set --port 5273`
...
- `application.properties` میں ماڈل کا نام بالکل درست اپڈیٹ کریں، بشمول ورژن نمبر (جیسے `Phi-3.5-mini-instruct-cuda-gpu:1`)
- اس بات کو یقینی بنائیں کہ `base-url` آخر میں `/v1` شامل کرے: `http://localhost:5273/v1`
- اگر ضرورت ہو تو ماڈل ڈاؤنلوڈ کریں: `foundry model run phi-3.5-mini`

**"400 بیڈ ریکویسٹ" ایررز**
- تصدیق کریں کہ بیس URL میں `/v1` شامل ہے: `http://localhost:5273/v1`
- چیک کریں کہ ماڈل ID بالکل وہی ہے جو `foundry model list` میں دکھائی گئی ہے
- اس بات کو یقینی بنائیں کہ آپ کوڈ میں `maxTokens()` استعمال کر رہے ہیں، نہ کہ `maxCompletionTokens()`
