...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-03T20:11:07+00:00",
...
**רעיון מרכזי:** Spring Boot טוען את ההגדרות הללו באופן אוטומטי ומאפשר גישה אליהן באפליקציה באמצעות האנומציה `@Value`.
...
- **ChatCompletionCreateParams**: מגדיר את בקשת ה-AI
  ...
  - `maxTokens`: מגביל את אורך התגובה (חוסך משאבים)
...
- **openai-java**: SDK Java רשמי של OpenAI לתקשורת API
...
2. **יצירת שירות**: Spring יוצר את `FoundryLocalService` ומזריק ערכי הגדרות
...
8. **תצוגה**: האפליקציה מדפיסה את התגובה ויוצאת
...
2. **בדיקת הפורט שהוקצה באופן דינמי**. Foundry Local מקצה פורט באופן אוטומטי כאשר הוא מופעל. מצאו את הפורט שלכם עם:
   ```bash
...
3. **הורדת מודל ה-AI** שברצונכם להשתמש בו, לדוגמה, `phi-3.5-mini`, עם הפקודה הבאה:
   ```bash
   foundry model run phi-3.5-mini
   ```


4. **הגדרת קובץ application.properties** כך שיתאים להגדרות Foundry Local שלכם:
...
- לחלופין, הגדירו פורט ספציפי אם רצוי: `foundry service set --port 5273`
- נסו להפעיל מחדש את Foundry Local: `foundry model run phi-3.5-mini`

**"Model not found" או שגיאות "404 Not Found"**
- בדקו את המודלים הזמינים עם מזהי המודל המדויקים שלהם: `foundry model list`
...
- ודאו שאתם משתמשים ב-`maxTokens()` במקום `maxCompletionTokens()` בקוד שלכם
