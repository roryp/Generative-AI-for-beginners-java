...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-23T12:25:47+00:00",
...
- לבחור ולהגדיר את סביבת הפיתוח המועדפת עליכם (ענן עם Codespaces, מכולת פיתוח מקומית, או התקנה מקומית מלאה)
...
פרק זה ינחה אתכם בהגדרת סביבת פיתוח. נשתמש ב-**GitHub Models** כדוגמה ראשית מכיוון שהוא חינמי, קל להגדרה עם חשבון GitHub בלבד, אינו דורש כרטיס אשראי, ומספק גישה למספר מודלים לניסויים.
...
אנו ממליצים להשתמש ב-[**GitHub Models**](https://github.com/marketplace?type=models) עבור קורס זה מכיוון שהוא:
- **חינמי** להתחלה
- **קל** להגדרה עם חשבון GitHub בלבד
- **אינו דורש כרטיס אשראי**
...
> - חינם ל-60 שעות בחודש לחשבונות אישיים
...
> **טיפ מקצועי**: אם יש לכם מחשב עם מפרט נמוך אך אתם רוצים להשתמש ב-VS Code מקומי, השתמשו ב-GitHub Codespaces! ניתן לחבר את VS Code המקומי שלכם ל-Codespace בענן כדי ליהנות מהטוב שבשני העולמות.
...
לאחר שסביבת הפיתוח שלכם מוכנה, בואו נבדוק את האינטגרציה עם GitHub Models באמצעות יישום הדוגמה שלנו ב-[`02-SetupDevEnvironment/src/github-models`](../../../02-SetupDevEnvironment/src/github-models).
...
ראשית, בואו נבין מה הרצנו. הדוגמה תחת `src/github-models` משתמשת ב-SDK של OpenAI ל-Java כדי להתחבר ל-GitHub Models:
...
- **לבדוק את האינטגרציה** עם יישום דוגמה עובד שמתקשר עם מודלי AI
...
- **האסימון לא עובד?** 
  - ודאו שהעתקתם את האסימון במלואו ללא רווחים נוספים
