...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-25T08:51:03+00:00",
  "source_file": "04-PracticalSamples/foundrylocal/README.md",
  "language_code": "fa"
}
-->
# برنامه خط فرمان محلی Foundry
...
- بهترین روش‌ها برای توسعه و آزمایش هوش مصنوعی محلی
...
- [عملکرد برنامه](../../../../04-PracticalSamples/foundrylocal)
...
قبل از اجرای این برنامه، باید Foundry Local را نصب و اجرا کنید. مراحل زیر را دنبال کنید:
...
1. **اطمینان حاصل کنید که سیستم شما الزامات را برآورده می‌کند**:
   ...
   - **سخت‌افزار**: 
     ...
     - توصیه‌شده: 16 گیگابایت رم، 15 گیگابایت فضای دیسک آزاد
   - **شبکه**: اتصال اینترنت برای دانلود اولیه مدل (اختیاری برای استفاده آفلاین)
   - **شتاب‌دهی (اختیاری)**: کارت گرافیک NVIDIA (سری 2000 یا جدیدتر)، کارت گرافیک AMD (سری 6000 یا جدیدتر)، Qualcomm Snapdragon X Elite (با حداقل 8 گیگابایت حافظه)، یا Apple silicon
...
برنامه را می‌توان از طریق `application.properties` پیکربندی کرد:
...
> **توجه**: نام مدل در پیکربندی باید با نسخه خاصی که Foundry Local برای سیستم شما دانلود کرده مطابقت داشته باشد. هنگامی که `foundry model run phi-3.5-mini` را اجرا می‌کنید، Foundry Local به طور خودکار بهترین نسخه را انتخاب و دانلود می‌کند (CUDA برای کارت‌های گرافیک NVIDIA، نسخه CPU در غیر این صورت). از `foundry model list` استفاده کنید تا نام دقیق مدل موجود در نمونه محلی خود را مشاهده کنید.
...
هنگامی که برنامه را اجرا می‌کنید:

1. **به عنوان یک برنامه خط فرمان** راه‌اندازی می‌شود (بدون سرور وب)
2. **به طور خودکار یک پیام آزمایشی ارسال می‌کند**: "سلام! می‌توانید به من بگویید که چه هستید و چه مدلی اجرا می‌کنید؟"
3. **پاسخ را از Foundry Local** در کنسول نمایش می‌دهد
4. **پس از نمایش دمو به طور پاکیزه خارج می‌شود**
...
- سریال‌سازی/دی‌سریال‌سازی JSON به طور خودکار توسط SDK مدیریت می‌شود
...
اگر خطاهای اتصال مشاهده کردید:
1. تأیید کنید که Foundry Local روی `http://localhost:5273` اجرا می‌شود
2. بررسی کنید که یک نسخه مدل Phi-3.5-mini با `foundry model list` موجود باشد
3. اطمینان حاصل کنید که نام مدل در `application.properties` با نام دقیق مدل موجود در لیست مطابقت دارد
4. مطمئن شوید که هیچ فایروالی اتصال را مسدود نمی‌کند

مشکلات رایج:
- **مدل یافت نشد**: `foundry model run phi-3.5-mini` را اجرا کنید تا مدل دانلود و اجرا شود
- **سرویس اجرا نمی‌شود**: ممکن است سرویس Foundry Local متوقف شده باشد؛ آن را با دستور اجرای مدل مجدداً راه‌اندازی کنید
- **نام مدل اشتباه است**: از `foundry model list` استفاده کنید تا مدل‌های موجود را مشاهده کرده و پیکربندی خود را به‌روزرسانی کنید.
