...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-25T08:46:42+00:00",
  "source_file": "04-PracticalSamples/foundrylocal/README.md",
  "language_code": "ru"
}
-->
# Локальное командное приложение Foundry

>**Примечание**: Эта глава включает [**Руководство**](./TUTORIAL.md), которое проведет вас через примеры.

Простое командное приложение Spring Boot, демонстрирующее, как подключиться к Foundry Local с использованием OpenAI Java SDK.

## Чему вы научитесь

- Как интегрировать Foundry Local с приложениями Spring Boot с помощью OpenAI Java SDK
...
> **⚠️ Примечание**: Это приложение **не работает в предоставленном devcontainer**, так как требует установки и запуска Foundry Local на хост-системе.
...
1. **Убедитесь, что ваша система соответствует требованиям**:
   ...
   - **Аппаратное обеспечение**: 
     - Минимум: 8 ГБ ОЗУ, 3 ГБ свободного места на диске
     - Рекомендуется: 16 ГБ ОЗУ, 15 ГБ свободного места на диске
   - **Сеть**: Подключение к интернету для первоначальной загрузки модели (опционально для автономного использования)
   ...
   - **Права**: Административные права для установки программного обеспечения на вашем устройстве
...
2. **Установите Foundry Local**:
   ...
   **Для macOS:**
   ...
   Кроме того, вы можете скачать установщик из [репозитория Foundry Local на GitHub](https://github.com/microsoft/Foundry-Local).
...
3. **Запустите первую модель**:
   ...
   Модель загружается (это может занять несколько минут в зависимости от скорости интернета) и затем запускается. Foundry Local автоматически выбирает лучший вариант модели для вашей системы (CUDA для NVIDIA GPU, версия для CPU в противном случае).
...
Вы также можете открыть `http://localhost:5273` в браузере, чтобы увидеть веб-интерфейс Foundry Local.
...
- `foundry.local.model` - Модель ИИ для использования (по умолчанию: Phi-3.5-mini-instruct-cuda-gpu)

> **Примечание**: Имя модели в конфигурации должно соответствовать конкретному варианту, который Foundry Local загрузил для вашей системы. Когда вы запускаете `foundry model run phi-3.5-mini`, Foundry Local автоматически выбирает и загружает лучший вариант (CUDA для NVIDIA GPU, версия для CPU в противном случае). Используйте `foundry model list`, чтобы увидеть точное имя модели, доступное в вашей локальной установке.

## Быстрый старт

### 1. Перейдите в каталог приложения Foundry Local
```bash
...
1. **Запускается** как командное приложение (без веб-сервера)
2. **Автоматически отправляет** тестовое сообщение: "Привет! Можешь рассказать, что ты такое и какую модель ты используешь?"
...
- Использует **OpenAI Java SDK** для типобезопасных API-вызовов
...
Создание запросов на завершение чата просто и типобезопасно:
...
3. Убедитесь, что имя модели в `application.properties` соответствует точному имени модели, указанному в списке
...
- **Неправильное имя модели**: Используйте `foundry model list`, чтобы увидеть доступные модели, и обновите конфигурацию соответственно
