...
5. Подождите примерно 2 минуты, пока окружение подготовится
...
## Структура курса и образовательный путь
...
- **Генератор историй про питомцев** (`petstory/`): творческая генерация контента с использованием GitHub Models
- **Демо Foundry локально** (`foundrylocal/`): локальная интеграция модели ИИ с OpenAI Java SDK
