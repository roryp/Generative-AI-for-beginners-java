...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-21T18:33:38+00:00",
...
- 安裝了 Java 21 或更高版本
- 使用 Maven 進行依賴管理
- 擁有一個 GitHub 帳戶，並配置了具有 `models:read` 權限的個人訪問令牌 (PAT)
...
5. **模型綁定：** 使用 Spring 的 `Model` 將資料傳遞給 HTML 模板

**備用系統：**
控制器包含預先編寫的故事模板，當 AI 服務不可用時會使用這些模板：
...
1. **Thymeleaf 整合：** 使用 `th:` 屬性進行動態內容渲染
2. **響應式設計：** 提供適用於行動裝置和桌面的 CSS 樣式
3. **錯誤處理：** 向用戶顯示驗證錯誤
...
1. **檔案上傳：** 允許上傳最大 10MB 的圖像
2. **日誌記錄：** 控制執行期間記錄的資訊
3. **GitHub Models：** 指定使用的 AI 模型和端點
4. **安全性：** 配置錯誤處理以避免暴露敏感資訊
...
2. 使用預先編寫的模板生成備用故事
3. 顯示備用故事並提示 AI 不可用
4. 確保使用者仍能獲得故事，提升使用體驗

## 了解 AI 整合

### GitHub Models API
應用程式使用 GitHub Models，提供免費訪問多種 AI 模型的功能：
...
欲了解更多範例，請參閱 [第 4 章：實用範例](../README.md)
