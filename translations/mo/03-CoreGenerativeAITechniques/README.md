...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-25T10:52:23+00:00",
...
此範例展示了如何通過 OpenAI API 與大型語言模型（LLM）互動的核心機制，包括使用 GitHub Models 初始化客戶端、系統與用戶提示的訊息結構模式、通過累積訊息歷史管理對話狀態，以及調整參數以控制回應長度與創意水平。
...
函數調用使 AI 模型能夠通過結構化協議請求執行外部工具和 API。模型分析自然語言請求，根據 JSON Schema 定義確定所需的函數調用及其參數，並處理返回結果以生成上下文回應，而實際的函數執行由開發者控制，以確保安全性與可靠性。
...
檢索增強生成（RAG）結合了信息檢索與語言生成，通過將外部文件內容注入 AI 提示，使模型能基於特定知識來源提供準確答案，而非依賴可能過時或不準確的訓練數據。同時，通過策略性提示設計，清楚區分用戶查詢與權威信息來源。
...
負責任的 AI 範例展示了在 AI 應用中實施安全措施的重要性。範例展示了檢測有害內容（如仇恨言論、騷擾、自殘、色情內容和暴力）的安全過濾器，並演示了如何通過適當的例外處理、用戶反饋機制和備選回應策略，優雅地處理內容政策違規情況。
