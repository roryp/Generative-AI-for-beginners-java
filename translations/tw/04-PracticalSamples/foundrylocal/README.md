...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-03T20:00:18+00:00",
...
**此配置的作用:**
- **base-url**: 指定 Foundry Local 運行的位置，包括 `/v1` 路徑以兼容 OpenAI API。**注意**: Foundry Local 會動態分配埠，因此請使用 `foundry service status` 檢查實際埠號。
- **model**: 指定用於文字生成的 AI 模型名稱，包括版本號（例如 `:1`）。使用 `foundry model list` 查看可用模型及其精確 ID。
...
- `WebApplicationType.NONE` 告訴 Spring 這是一個命令列應用程式，而不是 Web 伺服器
- 主方法啟動 Spring 應用程式
...
- 從 `application.properties` 中獲取的 base URL 已包含 `/v1` 以兼容 OpenAI API
- API 金鑰設置為 "not-needed"，因為本地開發不需要身份驗證
...
- **ChatCompletionCreateParams**: 配置 AI 請求
  - `model`: 指定要使用的 AI 模型（必須與 `foundry model list` 中的精確 ID 匹配）
  - `addUserMessage`: 將您的訊息添加到對話中
  - `maxTokens`: 限制回應的長度（節省資源）
...
要設置 Foundry Local，請按照以下步驟操作：

1. **安裝 Foundry Local**，請參考 [先決條件](../../../../04-PracticalSamples/foundrylocal) 部分中的指示。

2. **檢查動態分配的埠**。Foundry Local 啟動時會自動分配埠。使用以下命令找到您的埠：
   ```bash
   foundry service status
   ```
   
   **可選**: 如果您希望使用特定埠（例如 5273），可以手動配置：
   ```bash
...
4. **配置 application.properties** 檔案以匹配您的 Foundry Local 設置：
   - 更新 `base-url` 中的埠（來自步驟 2），確保其末尾包含 `/v1`
...
- 檢查 Foundry Local 使用的實際埠: `foundry service status`
- 使用正確的埠更新您的 `application.properties`，確保 URL 以 `/v1` 結尾
- 或者，如果需要，可以設置特定埠: `foundry service set --port 5273`
- 嘗試重新啟動 Foundry Local: `foundry model run phi-3.5-mini`

**"Model not found" 或 "404 Not Found" 錯誤**
- 使用 `foundry model list` 檢查可用模型及其精確 ID
- 在 `application.properties` 中更新模型名稱以完全匹配，包括版本號（例如 `Phi-3.5-mini-instruct-cuda-gpu:1`）
...
- 確保在程式碼中使用 `maxTokens()` 而不是 `maxCompletionTokens()`
