...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-21T16:04:13+00:00",
...
此範例展示了通過 OpenAI API 與大型語言模型（LLM）互動的核心機制，包括使用 GitHub Models 初始化客戶端、系統與用戶提示的訊息結構模式、通過訊息歷史累積進行對話狀態管理，以及調整參數以控制回應長度與創意水平。
...
這段程式碼建立了使用您的令牌連接到 GitHub Models 的連線。
...
AI 只有在您將先前的訊息包含在後續請求中時，才會記住之前的對話。
...
函數調用使 AI 模型能夠通過結構化協議請求執行外部工具和 API。模型分析自然語言請求，根據 JSON Schema 定義確定所需的函數調用及其參數，並處理返回的結果以生成上下文回應，而實際的函數執行由開發者控制，以確保安全性和可靠性。
...
檢索增強生成（RAG）結合了信息檢索與語言生成，通過將外部文件內容注入 AI 提示，使模型能夠基於特定知識來源提供準確答案，而非依賴可能過時或不準確的訓練數據。同時，通過策略性提示設計，保持用戶查詢與權威信息來源之間的清晰界限。
...
負責任的 AI 範例展示了在 AI 應用中實施安全措施的重要性。它展示了檢測有害內容類別（包括仇恨言論、騷擾、自殘、色情內容和暴力）的安全過濾器，並演示了生產環境中的 AI 應用如何通過適當的異常處理、用戶反饋機制和備選回應策略，優雅地處理內容政策違規情況。
...
程式測試各種有害提示，展示 AI 安全系統如何：
1. **阻止危險請求**，返回 HTTP 400 錯誤
2. **允許安全內容**正常生成
3. **保護用戶**免受有害 AI 輸出的影響

## 範例中的常見模式

### 驗證模式
所有範例都使用此模式與 GitHub Models 驗證：
