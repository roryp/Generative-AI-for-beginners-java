...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-21T19:56:40+00:00",
...
Αυτή η υπηρεσία υπολογιστή δείχνει πώς τα μοντέλα τεχνητής νοημοσύνης μπορούν να αλληλεπιδρούν με ασφάλεια με εξωτερικά εργαλεία χρησιμοποιώντας το MCP. Αντί να βασιζόμαστε στις, μερικές φορές, αναξιόπιστες μαθηματικές δυνατότητες του μοντέλου, θα σας δείξουμε πώς να δημιουργήσετε ένα αξιόπιστο σύστημα όπου η τεχνητή νοημοσύνη μπορεί να καλεί εξειδικευμένες υπηρεσίες για ακριβείς υπολογισμούς.
...
Εργαζόμενοι με αυτό το παράδειγμα, θα κατανοήσετε:
- Πώς να δημιουργήσετε υπηρεσίες συμβατές με MCP χρησιμοποιώντας Spring Boot
...
- Βέλτιστες πρακτικές για την ανάπτυξη εφαρμογών τεχνητής νοημοσύνης με δυνατότητα εργαλείων
...
- Java 21+
- Maven 3.6+
- **GitHub Token**: Απαιτείται για τον πελάτη με τεχνητή νοημοσύνη. Αν δεν το έχετε ρυθμίσει ακόμα, δείτε [Κεφάλαιο 2: Ρύθμιση του περιβάλλοντος ανάπτυξης](../../../02-SetupDevEnvironment/README.md) για οδηγίες.

## Βασικές Έννοιες

**Model Context Protocol (MCP)** είναι ένας τυποποιημένος τρόπος για τις εφαρμογές τεχνητής νοημοσύνης να συνδέονται με ασφάλεια με εξωτερικά εργαλεία. Σκεφτείτε το ως μια "γέφυρα" που επιτρέπει στα μοντέλα τεχνητής νοημοσύνης να χρησιμοποιούν εξωτερικές υπηρεσίες, όπως ο υπολογιστής μας. Αντί το μοντέλο τεχνητής νοημοσύνης να προσπαθεί να κάνει μαθηματικούς υπολογισμούς μόνο του (που μπορεί να είναι αναξιόπιστοι), μπορεί να καλέσει την υπηρεσία υπολογιστή μας για να πάρει ακριβή αποτελέσματα. Το MCP διασφαλίζει ότι αυτή η επικοινωνία γίνεται με ασφάλεια και συνέπεια.

**Server-Sent Events (SSE)** επιτρέπει την επικοινωνία σε πραγματικό χρόνο μεταξύ του διακομιστή και των πελατών. Σε αντίθεση με τις παραδοσιακές αιτήσεις HTTP, όπου ζητάτε και περιμένετε απάντηση, το SSE επιτρέπει στον διακομιστή να στέλνει συνεχώς ενημερώσεις στον πελάτη. Αυτό είναι ιδανικό για εφαρμογές τεχνητής νοημοσύνης όπου οι απαντήσεις μπορεί να μεταδίδονται ή να απαιτούν χρόνο για επεξεργασία.

**Εργαλεία Τεχνητής Νοημοσύνης & Κλήση Λειτουργιών** επιτρέπουν στα μοντέλα τεχνητής νοημοσύνης να επιλέγουν και να χρησιμοποιούν αυτόματα εξωτερικές λειτουργίες (όπως λειτουργίες υπολογιστή) με βάση τα αιτήματα των χρηστών. Όταν ρωτάτε "Πόσο κάνει 15 + 27;", το μοντέλο τεχνητής νοημοσύνης καταλαβαίνει ότι θέλετε πρόσθεση, καλεί αυτόματα το εργαλείο `add` με τις σωστές παραμέτρους (15, 27) και επιστρέφει το αποτέλεσμα σε φυσική γλώσσα. Η τεχνητή νοημοσύνη λειτουργεί ως ένας έξυπνος συντονιστής που ξέρει πότε και πώς να χρησιμοποιεί κάθε εργαλείο.
...
Δοκιμάζει την ακατέργαστη επικοινωνία πρωτοκόλλου MCP. Εκτελέστε:
```bash
...
Το MCP Inspector παρέχει ένα οπτικό web interface για να δοκιμάσετε την υπηρεσία MCP σας χωρίς να γράψετε κώδικα. Ιδανικό για αρχάριους που θέλουν να κατανοήσουν πώς λειτουργεί το MCP!

### Οδηγίες Βήμα-Βήμα:

1. **Εκκινήστε τον διακομιστή υπολογιστή** (αν δεν είναι ήδη σε λειτουργία):
   ```bash
   java -jar target/calculator-server-0.0.1-SNAPSHOT.jar
   ```
...
3. **Ανοίξτε το web interface**:
   - Αναζητήστε ένα μήνυμα όπως "Inspector running at http://localhost:6274"
   - Ανοίξτε αυτό το URL στον web browser σας

4. **Συνδεθείτε με την υπηρεσία υπολογιστή σας**:
   - Στο web interface, ορίστε τον τύπο μεταφοράς σε "SSE"
...
6. **Δοκιμάστε μια λειτουργία υπολογιστή**:
   ...
   - Πατήστε "Run Tool"
   - Δείτε το αποτέλεσμα που επιστρέφει η υπηρεσία MCP!
...
---
**Αναφορά:** [MCP Server Boot Starter Docs](https://docs.spring.io/spring-ai/reference/api/mcp/mcp-server-boot-starter-docs.html)
