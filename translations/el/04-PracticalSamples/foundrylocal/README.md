...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-03T20:07:39+00:00",
...
**Τι κάνει:**
- **base-url**: Καθορίζει πού εκτελείται το Foundry Local, συμπεριλαμβανομένου του μονοπατιού `/v1` για συμβατότητα με το OpenAI API. **Σημείωση**: Το Foundry Local εκχωρεί δυναμικά μια θύρα, οπότε ελέγξτε την πραγματική σας θύρα χρησιμοποιώντας `foundry service status`
- **model**: Ορίζει το όνομα του μοντέλου AI που θα χρησιμοποιηθεί για τη δημιουργία κειμένου, συμπεριλαμβανομένου του αριθμού έκδοσης (π.χ., `:1`). Χρησιμοποιήστε το `foundry model list` για να δείτε διαθέσιμα μοντέλα με τα ακριβή τους IDs.
...
- Το base URL από το `application.properties` περιλαμβάνει ήδη το `/v1` για συμβατότητα με το OpenAI API
...
- **ChatCompletionCreateParams**: Ρυθμίζει το αίτημα προς την AI
  - `model`: Καθορίζει ποιο μοντέλο AI θα χρησιμοποιηθεί (πρέπει να ταιριάζει με το ακριβές ID από το `foundry model list`)
  - `addUserMessage`: Προσθέτει το μήνυμά σας στη συνομιλία
  - `maxTokens`: Περιορίζει το μήκος της απάντησης (εξοικονομεί πόρους)
  - `temperature`: Ελέγχει την τυχαιότητα (0.0 = ντετερμινιστικό, 1.0 = δημιουργικό)
- **Κλήση API**: Στέλνει το αίτημα στο Foundry Local
- **Διαχείριση Απάντησης**: Εξάγει με ασφάλεια την κειμενική απάντηση της AI
...
3. **Κατεβάστε το μοντέλο AI** που θέλετε να χρησιμοποιήσετε, για παράδειγμα, `phi-3.5-mini`, με την εξής εντολή:
   ```bash
   foundry model run phi-3.5-mini
   ```
...
- Ενημερώστε το `application.properties` με τη σωστή θύρα, διασφαλίζοντας ότι το URL τελειώνει με `/v1`
...
- Επαληθεύστε ότι το base URL περιλαμβάνει `/v1`: `http://localhost:5273/v1`
- Ελέγξτε ότι το ID του μοντέλου ταιριάζει ακριβώς με αυτό που εμφανίζεται στο `foundry model list`
- Βεβαιωθείτε ότι χρησιμοποιείτε `maxTokens()` αντί για `maxCompletionTokens()` στον κώδικά σας
