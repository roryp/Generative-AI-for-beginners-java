...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-25T09:31:19+00:00",
...
- [Κύρια Σημεία Κώδικα](../../../../04-PracticalSamples/foundrylocal)
  ...
  - [API Ολοκλήρωσης Συνομιλίας](../../../../04-PracticalSamples/foundrylocal)
- [Αντιμετώπιση Προβλημάτων](../../../../04-PracticalSamples/foundrylocal)

## Προαπαιτούμενα

> **⚠️ Σημείωση**: Αυτή η εφαρμογή **δεν εκτελείται στο παρεχόμενο devcontainer** καθώς απαιτεί το Foundry Local να είναι εγκατεστημένο και να λειτουργεί στο σύστημα υποδοχής.

### Εγκατάσταση του Foundry Local

Πριν εκτελέσετε αυτήν την εφαρμογή, πρέπει να εγκαταστήσετε και να ξεκινήσετε το Foundry Local. Ακολουθήστε αυτά τα βήματα:
...
1. **Βεβαιωθείτε ότι το σύστημά σας πληροί τις απαιτήσεις**:
   ...
   - **Δίκτυο**: Σύνδεση στο διαδίκτυο για αρχική λήψη μοντέλου (προαιρετικό για χρήση εκτός σύνδεσης)
   - **Επιτάχυνση (προαιρετική)**: NVIDIA GPU (σειρά 2,000 ή νεότερη), AMD GPU (σειρά 6,000 ή νεότερη), Qualcomm Snapdragon X Elite (8GB ή περισσότερη μνήμη) ή Apple silicon
   - **Δικαιώματα**: Διοικητικά δικαιώματα για εγκατάσταση λογισμικού στη συσκευή σας
...
3. **Ξεκινήστε το πρώτο σας μοντέλο**:
   ...
   Το μοντέλο κατεβαίνει (μπορεί να πάρει λίγα λεπτά, ανάλογα με την ταχύτητα του διαδικτύου σας) και στη συνέχεια εκτελείται. Το Foundry Local επιλέγει αυτόματα την καλύτερη παραλλαγή μοντέλου για το σύστημά σας (CUDA για NVIDIA GPUs, CPU έκδοση διαφορετικά).
...
Μπορείτε επίσης να επισκεφθείτε το `http://localhost:5273` στον περιηγητή σας για να δείτε τη διεπαφή ιστού του Foundry Local.

## Ρύθμιση

Η εφαρμογή μπορεί να ρυθμιστεί μέσω του `application.properties`:

- `foundry.local.base-url` - Βασικό URL για το Foundry Local (προεπιλογή: http://localhost:5273)
...
Αυτή η εφαρμογή χρησιμοποιεί το OpenAI Java SDK για επικοινωνία με το Foundry Local. Η βασική εξάρτηση είναι:
...
1. **Ξεκινά** ως εφαρμογή γραμμής εντολών (χωρίς διακομιστή ιστού)
2. **Αποστέλλει αυτόματα** ένα δοκιμαστικό μήνυμα: "Γεια! Μπορείς να μου πεις τι είσαι και ποιο μοντέλο εκτελείς;"
...
- Χρησιμοποιεί **OpenAI Java SDK** για ασφαλείς κλήσεις API
- Αυτόματη σειριοποίηση/αποσειριοποίηση JSON που χειρίζεται το SDK
- Καθαρή ρύθμιση χρησιμοποιώντας τις σημειώσεις `@Value` και `@PostConstruct` του Spring

## Κύρια Σημεία Κώδικα

### Ενσωμάτωση OpenAI Java SDK

Η εφαρμογή χρησιμοποιεί το OpenAI Java SDK για τη δημιουργία ενός πελάτη που έχει ρυθμιστεί για το Foundry Local:
...
### API Ολοκλήρωσης Συνομιλίας

Η δημιουργία αιτημάτων ολοκλήρωσης συνομιλίας είναι απλή και ασφαλής:
...
1. Επαληθεύστε ότι το Foundry Local εκτελείται στο `http://localhost:5273`
2. Ελέγξτε ότι μια παραλλαγή μοντέλου Phi-3.5-mini είναι διαθέσιμη με `foundry model list`
...
4. Βεβαιωθείτε ότι κανένα τείχος προστασίας δεν μπλοκάρει τη σύνδεση

Συνηθισμένα προβλήματα:
- **Μοντέλο δεν βρέθηκε**: Εκτελέστε `foundry model run phi-3.5-mini` για να κατεβάσετε και να ξεκινήσετε το μοντέλο
- **Υπηρεσία δεν εκτελείται**: Η υπηρεσία Foundry Local μπορεί να έχει σταματήσει. Επανεκκινήστε την με την εντολή εκτέλεσης μοντέλου
- **Λάθος όνομα μοντέλου**: Χρησιμοποιήστε `foundry model list` για να δείτε διαθέσιμα μοντέλα και ενημερώστε τη ρύθμισή σας αναλόγως
